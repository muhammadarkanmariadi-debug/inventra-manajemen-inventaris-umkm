...
export default function InventoriesPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      setError(err?.message || _(msg`Gagal memuat data inventaris.`));
    } finally {
      setLoading(false);
    }
  }, [currentPage]);
  ...
  return (
    <div>
      ...
      {error && <div className="mb-4"><Alert variant="error" title={_(msg`Error`)} message={error} /></div>}
                      ...
                      <TableRow key={inv.id}>
                        ...
                        <TableCell className="px-4 py-3 text-start">
                          <div className="flex items-center gap-2">
                            <button onClick={() => handleViewDetail(inv.id)} className="text-blue-500 hover:text-blue-700 text-sm">Detail</button>
                            <button onClick={() => router.push(`/scan?code=${inv.inventory_code}`)} className="text-brand-500 hover:text-brand-700 text-sm">Scan</button>
        ...
        <div className="p-5">
          ...
          {detailLoading ? (
            ...
            <div className="space-y-4">
              ...
              {/* Event Logs */}
              {detailItem.logs && detailItem.logs.length > 0 && (
                <div>
                  <h5 className="mb-2 text-sm font-semibold text-gray-700 dark:text-gray-300">Riwayat Event</h5>
                  <div className="max-h-60 overflow-y-auto rounded-lg border border-gray-100 dark:border-gray-700">
                    <table className="w-full text-xs">
                      <thead className="bg-gray-50 dark:bg-gray-800">
                        <tr>
                          <th className="px-3 py-2 text-left text-gray-500">Aksi</th>
                          ...
                          <th className="px-3 py-2 text-left text-gray-500">Waktu</th>
                        </tr>
                      </thead>
                      <tbody>
                        {detailItem.logs.map(log => (
                          <tr key={log.id} className="border-t border-gray-100 dark:border-gray-700">
                            <td className="px-3 py-2">
                              <Badge size="sm" color={log.action === 'SCAN' ? 'primary' : log.action === 'STATUS_CHANGE' ? 'warning' : 'light'}>
                                {log.action}
                              </Badge>
                            </td>
                            <td className="px-3 py-2 text-gray-600 dark:text-gray-400">
                              {log.from_status?.code && log.to_status?.code
                                ? `${log.from_status.code} → ${log.to_status.code}`
                                : log.to_status?.code || '-'}
                            </td>
                            <td className="px-3 py-2 text-gray-600 dark:text-gray-400">{log.location?.name || '-'}</td>
                            <td className="px-3 py-2 text-gray-600 dark:text-gray-400">{log.user?.name || '-'}</td>
                            <td className="px-3 py-2 text-gray-500">{new Date(log.created_at).toLocaleString('id-ID')}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
