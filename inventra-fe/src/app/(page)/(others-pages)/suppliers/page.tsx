...
export default function SuppliersPage() {
  ...
  const [formData, setFormData] = useState<CreateSupplierPayload>({ name: '', contact_info: '' });
  ...
  const fetchSuppliers = useCallback(async () => {
    ...
    try {
      ...
      if (res.status) {
        ...
        setSuppliers([]);
        setError(res.message || 'Gagal memuat data supplier');
      }
    } catch {
  ...
  const resetForm = () => { setFormData({ name: '', contact_info: '' }); setEditingSupplier(null); };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res.status) {
        ...
        setError(res.message || 'Gagal menyimpan supplier');
      }
    } catch {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (res.status) {
        ...
        setError(res.message || 'Gagal menghapus supplier');
      }
    } catch {
  ...
  return (
    <div>
      ...
      <div className="mb-4 flex justify-end">
        <Button size="sm" onClick={openCreateModal}>+ Tambah Supplier</Button>
      </div>
          ...
          <Table>
            ...
            <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
              {loading ? (
                ...
                <TableRow><TableCell className="px-5 py-8 text-center text-gray-500">Tidak ada data supplier</TableCell></TableRow>
              ) : (
                suppliers.map((sup) => (
                  <TableRow key={sup.id}>
                    ...
                    <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">{sup.contact_info || '-'}</TableCell>
                    <TableCell className="px-4 py-3 text-start">
                      <div className="flex items-center gap-2">
                        <button onClick={() => openEditModal(sup)} className="text-brand-500 hover:text-brand-700 text-sm">Edit</button>
                        <button onClick={() => openDeleteModal(sup)} className="text-error-500 hover:text-error-700 text-sm">Hapus</button>
                      </div>
                    </TableCell>
                  </TableRow>
                ))
      ...
      <Modal isOpen={showFormModal} onClose={() => { setShowFormModal(false); resetForm(); }} className="max-w-md p-6 lg:p-10">
        ...
        <div className="space-y-4">
          <div>
            ...
            <Input type="text" placeholder="Nama supplier" defaultValue={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} />
          </div>
          <div>
            <Label>Info Kontak</Label>
            <TextArea placeholder="Nomor telepon, email, dll (opsional)" value={formData.contact_info || ''} onChange={(val) => setFormData({ ...formData, contact_info: val })} rows={3} />
          </div>
        </div>
      ...
      <Modal isOpen={showDeleteModal} onClose={() => setShowDeleteModal(false)} className="max-w-sm p-6">
        <h4 className="mb-2 text-lg font-semibold text-gray-800 dark:text-white/90">Hapus Supplier</h4>
