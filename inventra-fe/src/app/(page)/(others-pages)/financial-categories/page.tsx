...
export default function FinancialCategoriesPage() {
  ...
  const fetchCategories = useCallback(async () => {
    ...
    try {
      ...
      if (res.status) {
        ...
        setCategories([]);
        setError(res.message || 'Gagal memuat data kategori keuangan');
      }
    } catch {
      setError('Gagal memuat data kategori keuangan');
    } finally {
      setLoading(false);
    }
  }, [currentPage]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res.status) {
        setSuccessMsg(editingCategory ? 'Kategori keuangan berhasil diperbarui' : 'Kategori keuangan berhasil ditambahkan');
        setShowFormModal(false);
        resetForm();
        fetchCategories();
        setTimeout(() => setSuccessMsg(''), 3000);
      } else {
        setError(res.message || 'Gagal menyimpan kategori keuangan');
      }
    } catch {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (res.status) {
        setSuccessMsg('Kategori keuangan berhasil dihapus');
        setShowDeleteModal(false);
        setDeletingCategory(null);
        fetchCategories();
        setTimeout(() => setSuccessMsg(''), 3000);
      } else {
        setError(res.message || 'Gagal menghapus kategori keuangan');
      }
    } catch {
    ...
    <div>
          ...
          <Table>
            ...
            <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
              {loading ? (
                ...
                <TableRow><TableCell className="px-5 py-8 text-center text-gray-500">Tidak ada data kategori keuangan</TableCell></TableRow>
              ) : (
                categories.map((cat) => (
                  <TableRow key={cat.id}>
      ...
      <Modal isOpen={showFormModal} onClose={() => { setShowFormModal(false); resetForm(); }} className="max-w-md p-6 lg:p-10">
        <h4 className="mb-6 text-lg font-semibold text-gray-800 dark:text-white/90">
          {editingCategory ? 'Edit Kategori Keuangan' : 'Tambah Kategori Keuangan'}
        </h4>
      ...
      <Modal isOpen={showDeleteModal} onClose={() => setShowDeleteModal(false)} className="max-w-sm p-6">
        <h4 className="mb-2 text-lg font-semibold text-gray-800 dark:text-white/90">Hapus Kategori Keuangan</h4>
