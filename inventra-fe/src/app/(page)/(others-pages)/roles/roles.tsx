...
export default function Roles() {
  ...
  const fetchRoles = useCallback(async () => {
    ...
    try {
      ...
      if (res.status) {
        ...
        setRoles([]);
        setError(res.message || _(msg`Gagal memuat data role`));
      }
    } catch {
      setError(_(msg`Gagal memuat data role`));
    } finally {
      setLoading(false);
    }
  }, [currentPage]);
  ...
  const openEditModal = async (role: Role) => {
    try {
      ...
      setError(_(msg`Gagal memuat detail role`));
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res.status) {
        setSuccessMsg(editingRole ? _(msg`Role berhasil diperbarui`) : _(msg`Role berhasil ditambahkan`));
        setShowFormModal(false);
        resetForm();
        fetchRoles();
        setTimeout(() => setSuccessMsg(''), 3000);
      } else {
        setError(res.message || _(msg`Gagal menyimpan role`));
      }
    } catch {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (res.status) {
        setSuccessMsg(_(msg`Role berhasil dihapus`));
        setShowDeleteModal(false);
        setDeletingRole(null);
        fetchRoles();
        setTimeout(() => setSuccessMsg(''), 3000);
      } else {
        setError(res.message || _(msg`Gagal menghapus role`));
      }
    } catch {
                    ...
                    <TableCell className="px-4 py-3 text-start">
                      <div className="flex items-center gap-2">
                        ...
                        <Can permission="roles.update">
                          <button onClick={() => openEditModal(role)} className="text-brand-500 hover:text-brand-700 text-sm"><Trans id="Edit" /></button>
                        </Can>
                        <Can permission="roles.delete">
                          <button onClick={() => openDeleteModal(role)} className="text-error-500 hover:text-error-700 text-sm"><Trans id="Hapus" /></button>
                        </Can>
                      </div>
                    </TableCell>
                  </TableRow>
