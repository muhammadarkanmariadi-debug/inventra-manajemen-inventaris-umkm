...
const QUICK_QUESTIONS = [
  ...
  'Produk apa yang perlu di-restock?',
  'Berikan analisis keuntungan bisnis saya',
...
export default function ChatbotPage() {
  const [messages, setMessages] = useState<Message[]>([
    {
      id: 0,
      role: 'assistant',
      content: 'Halo! 👋 Saya adalah asisten AI Inventra. Saya bisa membantu Anda menganalisis data bisnis:\n\n• 📦 Stok & inventori\n• 💰 Keuangan & profit\n• 📊 Penjualan & tren\n\nSilakan tanyakan apa saja tentang bisnis Anda!',
      timestamp: new Date(),
    },
  ]);
  ...
  const sendMessage = async (question?: string) => {
    ...
    try {
      ...
      if (res.status) {
        ...
        aiText = res.message || 'Maaf, saya tidak bisa menjawab saat ini.';
      }
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-140px)]">
      ...
      {error && <div className="mb-2"><Alert variant="error" title="Error" message={error} /></div>}
      ...
      <div className="flex-1 flex flex-col rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03] overflow-hidden">
        ...
        <div className="border-t border-gray-200 p-4 dark:border-gray-800">
          ...
          <p className="mt-2 text-xs text-gray-400 text-center">
            AI menganalisis data produk, penjualan, dan keuangan bisnis Anda secara real-time
          </p>
        </div>
      </div>
    </div>
