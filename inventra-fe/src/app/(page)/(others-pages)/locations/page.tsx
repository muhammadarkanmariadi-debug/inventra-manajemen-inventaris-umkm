...
export default function LocationsPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editingLocation) {
        ...
        setSuccessMsg('Gudang berhasil diperbarui.');
      } else {
        await createLocation(formName.trim());
        setSuccessMsg('Gudang berhasil ditambahkan.');
      }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setSuccessMsg('Gudang berhasil dihapus.');
      setShowDeleteModal(false);
      setDeletingLocation(null);
      fetchData();
    } catch (err: any) {
      setError(err?.message || 'Gagal menghapus Gudang.');
    } finally {
      setSubmitting(false);
    }
  };
                      ...
                      <TableRow key={loc.id}>
                        ...
                        <TableCell className="px-4 py-3 text-start">
                          <div className="flex items-center gap-2">
                            <button onClick={() => openEditModal(loc)} className="text-brand-500 hover:text-brand-700 text-sm">Edit</button>
                            <button onClick={() => { setDeletingLocation(loc); setShowDeleteModal(true); }} className="text-error-500 hover:text-error-700 text-sm">Hapus</button>
                          </div>
                        </TableCell>
                      </TableRow>
                    ))
