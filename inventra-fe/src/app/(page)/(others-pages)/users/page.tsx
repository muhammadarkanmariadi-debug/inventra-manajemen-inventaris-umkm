  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (editingUser && !payload.password) {
        ...
        if (res.status) {
          setSuccessMsg(editingUser ? 'User berhasil diperbarui' : 'User berhasil ditambahkan');
          setShowFormModal(false);
          resetForm();
          fetchUsers();
          setTimeout(() => setSuccessMsg(''), 3000);
  ...
  return (
    <div>
      ...
      <div className="mb-4 flex justify-end">
        <Button size="sm" onClick={openCreateModal}>+ Tambah User</Button>
      </div>
        ...
        <div className="max-w-full overflow-x-auto">
          <Table>
            <TableHeader className="border-b border-gray-100 dark:border-white/[0.05]">
              <TableRow>
                {['Username', 'Email', 'Roles', 'Aksi'].map((h) => (
            ...
            <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
              {loading ? (
                ...
                <TableRow><TableCell className="px-5 py-8 text-center text-gray-500">Tidak ada data user</TableCell></TableRow>
              ) : (
                users.map((user) => (
                  <TableRow key={user.id}>
                    ...
                    <TableCell className="px-4 py-3 text-start">
                      <div className="flex items-center gap-2">
                        <button onClick={() => openEditModal(user)} className="text-brand-500 hover:text-brand-700 text-sm">Edit</button>
                        <button onClick={() => openDeleteModal(user)} className="text-error-500 hover:text-error-700 text-sm">Hapus</button>
                      </div>
                    </TableCell>
                  </TableRow>
                ))
      ...
      <Modal isOpen={showDeleteModal} onClose={() => setShowDeleteModal(false)} className="max-w-sm p-6">
        <h4 className="mb-2 text-lg font-semibold text-gray-800 dark:text-white/90">Hapus User</h4>
