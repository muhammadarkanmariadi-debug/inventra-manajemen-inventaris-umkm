...
export default function Sales() {
  ...
  const fetchSales = useCallback(async () => {
    ...
    try {
      ...
      if (res.status) {
        ...
        setSales([]);
        setError(res.message || _(msg`Gagal memuat data penjualan`));
      }
    } catch {
      setError(_(msg`Gagal memuat data penjualan`));
    } finally {
      setLoading(false);
    }
  }, [currentPage]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res.status) {
        ...
        setError(res.message || _(msg`Gagal menyimpan penjualan`));
      }
    } catch {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (res.status) {
        setSuccessMsg(_(msg`Penjualan berhasil dihapus`));
        setShowDeleteModal(false);
        setDeletingSale(null);
        fetchSales();
        setTimeout(() => setSuccessMsg(''), 3000);
      } else {
        setError(res.message || _(msg`Gagal menghapus penjualan`));
      }
    } catch {
    ...
    <div>
                    ...
                    <TableRow key={sale.id}>
                      ...
                      <TableCell className="px-4 py-3 text-start">
                        <div className="flex items-center gap-2">
                          ...
                          <button onClick={() => openDeleteModal(sale)} className="text-error-500 hover:text-error-700 text-sm"><Trans id="Hapus" /></button>
                        </div>
                      </TableCell>
                    </TableRow>
                  ))
      ...
      <Modal isOpen={showFormModal} onClose={() => { setShowFormModal(false); resetForm(); }} className="max-w-md p-6 lg:p-10">
        <h4 className="mb-6 text-lg font-semibold text-gray-800 dark:text-white/90">
          {editingSale ? <Trans id="Edit Penjualan" /> : <Trans id="Tambah Penjualan" />}
        </h4>
        <div className="space-y-4">
          <div>
            <Label><Trans id="Produk" /></Label>
            <Select
              options={products.map((p) => ({ value: String(p.id), label: `${p.name} (${_(msg`Stok`)}: ${p.stock})` }))}
