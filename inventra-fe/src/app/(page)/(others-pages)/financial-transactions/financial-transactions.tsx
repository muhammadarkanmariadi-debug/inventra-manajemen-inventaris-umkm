...
export default function FinancialTransactions() {
  ...
  const fetchTransactions = useCallback(async () => {
    ...
    try {
      ...
      if (res.status) {
        ...
        setTransactions([]);
        setError(res.message || _(msg`Gagal memuat data transaksi keuangan`));
      }
    } catch {
      setError(_(msg`Gagal memuat data transaksi keuangan`));
    } finally {
      setLoading(false);
    }
  }, [currentPage]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res.status) {
        setSuccessMsg(editingTx ? _(msg`Transaksi keuangan berhasil diperbarui`) : _(msg`Transaksi keuangan berhasil ditambahkan`));
        setShowFormModal(false);
        resetForm();
        fetchTransactions();
        setTimeout(() => setSuccessMsg(''), 3000);
      } else {
        setError(res.message || _(msg`Gagal menyimpan transaksi keuangan`));
      }
    } catch {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (res.status) {
        setSuccessMsg(_(msg`Transaksi keuangan berhasil dihapus`));
        setShowDeleteModal(false);
        setDeletingTx(null);
        fetchTransactions();
        setTimeout(() => setSuccessMsg(''), 3000);
      } else {
        setError(res.message || _(msg`Gagal menghapus transaksi keuangan`));
      }
    } catch {
                    ...
                    <TableRow key={tx.id}>
                      ...
                      <TableCell className="px-4 py-3 text-start">
                        <div className="flex items-center gap-2">
                          <button onClick={() => openEditModal(tx)} className="text-brand-500 hover:text-brand-700 text-sm"><Trans id="Edit" /></button>
                          <button onClick={() => openDeleteModal(tx)} className="text-error-500 hover:text-error-700 text-sm"><Trans id="Hapus" /></button>
                        </div>
                      </TableCell>
                    </TableRow>
                  ))
