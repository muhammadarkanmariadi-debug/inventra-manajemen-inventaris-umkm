...
export default function StockPredictionPage() {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      if (res.status) {
        ...
        setError(res.message || 'Gagal menganalisis stok');
      }
    } catch {
  ...
  const getStockStatus = (stock: number) => {
    if (stock === 0) return { label: 'Habis', color: 'error' as const, priority: 0 };
    if (stock <= 5) return { label: 'Kritis', color: 'error' as const, priority: 1 };
    if (stock <= 15) return { label: 'Rendah', color: 'warning' as const, priority: 2 };
    if (stock <= 50) return { label: 'Sedang', color: 'info' as const, priority: 3 };
    return { label: 'Aman', color: 'success' as const, priority: 4 };
  };
  ...
  return (
    <div>
      <PageBreadcrumb pageTitle="Prediksi & Analisis Stok" />
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3 mb-6">
        <div className="rounded-2xl border border-error-200 bg-error-50 p-5 dark:border-error-800 dark:bg-error-500/10">
          ...
          <p className="mt-1 text-sm text-error-600 dark:text-error-400">Produk Stok Kritis (≤5)</p>
        </div>
        <div className="rounded-2xl border border-warning-200 bg-warning-50 p-5 dark:border-warning-800 dark:bg-warning-500/10">
          ...
          <p className="mt-1 text-sm text-warning-600 dark:text-warning-400">Produk Stok Rendah (6-15)</p>
        </div>
        <div className="rounded-2xl border border-success-200 bg-success-50 p-5 dark:border-success-800 dark:bg-success-500/10">
          ...
          <p className="mt-1 text-sm text-success-600 dark:text-success-400">Produk Stok Aman ({'>'} 15)</p>
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/[0.03] mb-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h4 className="text-lg font-semibold text-gray-800 dark:text-white/90">🤖 Prediksi AI (Gemini)</h4>
            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">AI akan menganalisis inventori dan memberikan rekomendasi restock, risiko kehabisan stok, dan strategi pengelolaan</p>
          </div>
        ...
        {analyzing && (
          <div className="flex flex-col items-center justify-center py-12">
            ...
            <p className="text-sm text-gray-500">AI sedang menganalisis {products.length} produk...</p>
          </div>
        )}
        ...
        {!analysis && !analyzing && (
          <div className="flex items-center justify-center py-8 text-gray-400 text-sm">
            Klik &quot;Analisis Stok&quot; untuk mendapatkan prediksi dan rekomendasi dari AI
          </div>
        )}
      </div>
      ...
      <div className="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
        <div className="p-5 border-b border-gray-200 dark:border-gray-800">
          <h4 className="text-lg font-semibold text-gray-800 dark:text-white/90">Status Stok Seluruh Produk</h4>
        </div>
          ...
          <div className="min-w-[700px]">
            <Table>
              <TableHeader className="border-b border-gray-100 dark:border-white/[0.05]">
                <TableRow>
                  {['Produk', 'SKU', 'Kategori', 'Stok', 'Status', 'Tipe'].map((h) => (
