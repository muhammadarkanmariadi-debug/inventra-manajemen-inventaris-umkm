...
export default function StockTransactionsPage() {
  ...
  const fetchTransactions = useCallback(async () => {
    ...
    try {
      ...
      if (res.status) {
        ...
        setTransactions([]);
        setError(res.message || 'Gagal memuat data transaksi stok');
      }
    } catch {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res.status) {
        setSuccessMsg('Transaksi stok berhasil dibuat');
        setShowFormModal(false);
        resetForm();
        fetchTransactions();
        setTimeout(() => setSuccessMsg(''), 3000);
      } else {
        setError(res.message || 'Gagal menyimpan transaksi stok');
      }
    } catch {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (res.status) {
        setSuccessMsg('Transaksi stok berhasil dihapus');
        setShowDeleteModal(false);
        setDeletingTx(null);
        fetchTransactions();
        setTimeout(() => setSuccessMsg(''), 3000);
      } else {
        setError(res.message || 'Gagal menghapus transaksi stok');
      }
    } catch {
  ...
  return (
    <div>
      <PageBreadcrumb pageTitle="Transaksi Stok" />

      {successMsg && <div className="mb-4"><Alert variant="success" title="Berhasil" message={successMsg} /></div>}
      {error && <div className="mb-4"><Alert variant="error" title="Error" message={error} /></div>}

      <div className="mb-4 flex justify-end">
        <Button size="sm" onClick={() => { resetForm(); setShowFormModal(true); }}>+ Tambah Transaksi</Button>
      </div>
        ...
        <div className="max-w-full overflow-x-auto">
          <Table>
            <TableHeader className="border-b border-gray-100 dark:border-white/[0.05]">
              <TableRow>
                {['Produk', 'Tipe', 'Jumlah', 'Catatan', 'Tanggal', 'Aksi'].map((h) => (
            ...
            <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
              {loading ? (
                ...
                <TableRow><TableCell className="px-5 py-8 text-center text-gray-500">Tidak ada data transaksi stok</TableCell></TableRow>
              ) : (
                transactions.map((tx) => (
                  <TableRow key={tx.id}>
                    ...
                    <TableCell className="px-4 py-3 text-start">
                      <button onClick={() => { setDeletingTx(tx); setShowDeleteModal(true); }} className="text-error-500 hover:text-error-700 text-sm">Hapus</button>
                    </TableCell>
                  </TableRow>
                ))
              )}
      ...
      <Modal isOpen={showFormModal} onClose={() => { setShowFormModal(false); resetForm(); }} className="max-w-2xl p-6 lg:p-10">
        <h4 className="mb-6 text-lg font-semibold text-gray-800 dark:text-white/90">Tambah Transaksi Stok</h4>
          ...
          {formItems.map((item, index) => (
            <div key={index} className="rounded-lg border border-gray-200 p-4 dark:border-gray-700">
              <div className="flex items-center justify-between mb-3">
                ...
                {formItems.length > 1 && (
                  <button onClick={() => removeItem(index)} className="text-error-500 hover:text-error-700 text-xs">Hapus</button>
                )}
              </div>
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <Label>Catatan</Label>
                  <Input type="text" placeholder="Opsional" defaultValue={item.note || ''} onChange={(e) => updateItem(index, 'note', e.target.value)} />
                </div>
              </div>
            </div>
          ))}
        ...
        <div className="mt-4">
          <Button variant="outline" size="sm" onClick={addItem}>+ Tambah Item</Button>
        </div>
      ...
      <Modal isOpen={showDeleteModal} onClose={() => setShowDeleteModal(false)} className="max-w-sm p-6">
        <h4 className="mb-2 text-lg font-semibold text-gray-800 dark:text-white/90">Hapus Transaksi</h4>
        <p className="mb-6 text-sm text-gray-500 dark:text-gray-400">
          Apakah Anda yakin ingin menghapus transaksi stok ini? Stok produk akan dikembalikan sesuai tipe transaksi.
        </p>
