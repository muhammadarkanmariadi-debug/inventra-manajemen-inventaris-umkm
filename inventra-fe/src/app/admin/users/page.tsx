...
import { getAllUsers, createUser, updateAdminUser, deleteAdminUser } from '../../../../services/user.service';
...
export default function AdminUsers() {
  ...
  const fetchUsers = useCallback(async () => {
    ...
    try {
      ...
      if (res.status) {
        ...
        setUsers([]);
        toast.error(res.message || _("Gagal memuat data user"));
      }
    } catch {
      toast.error(_("Gagal memuat data user"));
    } finally {
      setLoading(false);
    }
  }, [currentPage, _]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (editingUser && !payload.password) {
        ...
        const res = await updateAdminUser(editingUser.id, rest); // use superadmin update
        if (res.status) {
          toast.success(_("User berhasil diperbarui"));
          setShowFormModal(false);
          resetForm();
          fetchUsers();
        } else {
          toast.error(res.message || _("Gagal menyimpan user"));
        }
      } else {
        ...
        if (res.status) {
          toast.success(editingUser ? _("User berhasil diperbarui") : _("User berhasil ditambahkan"));
          setShowFormModal(false);
          resetForm();
          fetchUsers();
        } else {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (res.status) {
        toast.success(_("User berhasil dihapus"));
        setShowDeleteModal(false);
        setDeletingUser(null);
        fetchUsers();
      } else {
        toast.error(res.message || _("Gagal menghapus user"));
      }
    } catch {
  ...
  const filterConfig = {
    ...
    searchPlaceholder: _("Cari user berdasarkan username atau email..."),
  };
  ...
  const handleExport = () => {
    const exportData = filteredUsers.map(user => ({
      Username: user.username,
      Email: user.email,
      Bisnis: user.business?.name || "Tidak ada",
      Roles: user.roles?.map(r => r.name).join(', ') || "-"
    }));
    exportToExcel(exportData, 'SaaS_Users');
  };

  return (
    <PermissionWrapper permission="Lihat Pengguna" breadcrumb="SaaS Users">
      <div>
      ...
      <div className='flex flex-col gap-4 mb-4'>
        ...
        <div className="flex justify-end gap-3">
          ...
          <Can permission="Tambah Pengguna">
            <Button size="sm" onClick={openCreateModal}>+ <Trans>Tambah User</Trans></Button>
          </Can>
        </div>
      </div>
          ...
          <Table>
            ...
            <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
              {loading ? (
                ...
                <TableRow><TableCell className="px-5 py-8 text-center text-gray-500" colSpan={5}><Trans>Tidak ada data user</Trans></TableCell></TableRow>
              ) : (
                filteredUsers.map((user) => (
                  <TableRow key={user.id}>
                    ...
                    <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">{user.business?.name || "Tidak ada"}</TableCell>
      ...
      <Modal isOpen={showFormModal} onClose={() => { setShowFormModal(false); resetForm(); }} className="max-w-md p-6 lg:p-10">
        <h4 className="mb-6 text-lg font-semibold text-gray-800 dark:text-white/90">
          {editingUser ? <Trans>Edit User</Trans> : <Trans>Tambah User</Trans>}
        </h4>
        <div className="space-y-4">
          ...
          <div>
            <Label>{editingUser ? <Trans>Password (kosongkan jika tidak diubah)</Trans> : 'Password'}</Label>
      ...
      <Modal isOpen={showDeleteModal} onClose={() => setShowDeleteModal(false)} className="max-w-sm p-6">
        <h4 className="mb-2 text-lg font-semibold text-gray-800 dark:text-white/90"><Trans>Hapus User</Trans></h4>
        <p className="mb-6 text-sm text-gray-500 dark:text-gray-400">
          <Trans>Apakah Anda yakin ingin menghapus user</Trans> <strong>{deletingUser?.username}</strong>?
        </p>
        <div className="flex justify-end gap-3">
