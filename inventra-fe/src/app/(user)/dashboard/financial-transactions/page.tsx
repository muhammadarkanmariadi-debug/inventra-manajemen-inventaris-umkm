...
export default function FinancialTransactions() {
  ...
  const fetchTransactions = useCallback(async () => {
    ...
    try {
      ...
      if (res.status) {
        ...
        setTransactions([]);
        toast.error(res.message || _("Gagal memuat data transaksi keuangan"));
      }
    } catch {
      toast.error(_("Gagal memuat data transaksi keuangan"));
    } finally {
      setLoading(false);
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res.status) {
        toast.success(editingTx ? _("Transaksi keuangan berhasil diperbarui") : _("Transaksi keuangan berhasil ditambahkan"));
        setShowFormModal(false);
        resetForm();
        fetchTransactions();
      } else {
        toast.error(res.message || _("Gagal menyimpan transaksi keuangan"));
      }
    } catch {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (res.status) {
        toast.success(_("Transaksi keuangan berhasil dihapus"));
        setShowDeleteModal(false);
        setDeletingTx(null);
        fetchTransactions();
      } else {
        toast.error(res.message || _("Gagal menghapus transaksi keuangan"));
      }
    } catch {
  ...
  const filterConfig = {
    tabs: [
      ...
      { label: _("Pemasukan"), value: "income" },
      { label: _("Pengeluaran"), value: "expense" }
    ],
    selects: [
      {
        label: _("Kategori"),
    ...
    <PermissionWrapper permission="Lihat Transaksi Keuangan" breadcrumb="Transaksi Keuangan">
      ...
      <div className='flex flex-col gap-4 mb-4'>
        ...
        <div className="flex justify-end gap-3">
          ...
          <Can permission="Tambah Transaksi Keuangan">
            <Button size="sm" onClick={openCreateModal}>+ <Trans>Tambah Transaksi</Trans></Button>
          </Can>
        </div>
      </div>
            ...
            <Table>
              ...
              <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
                {loading ? (
                  ...
                  <TableRow><TableCell className="px-5 py-8 text-center text-gray-500"><Trans>Tidak ada data transaksi keuangan</Trans></TableCell></TableRow>
                ) : (
                  filteredTransactions.map((tx) => (
                    <TableRow key={tx.id}>
                      <TableCell className="px-4 py-3 text-start">
                        <Badge size="sm" color={tx.type === 'income' ? 'success' : 'error'}>
                          {tx.type === 'income' ? <Trans>Pemasukan</Trans> : <Trans>Pengeluaran</Trans>}
                        </Badge>
                      </TableCell>
                      <TableCell className="px-4 py-3 text-start">
      ...
      <Modal isOpen={showFormModal} onClose={() => { setShowFormModal(false); resetForm(); }} className="max-w-md p-6 lg:p-10">
        <h4 className="mb-6 text-lg font-semibold text-gray-800 dark:text-white/90">
          {editingTx ? <Trans>Edit Transaksi Keuangan</Trans> : <Trans>Tambah Transaksi Keuangan</Trans>}
        </h4>
        <div className="space-y-4">
          <div>
            <Label><Trans>Kategori Keuangan</Trans></Label>
            <Select
              options={categories.map((c) => ({ value: String(c.id), label: `${c.name} (${c.type === 'income' ? _("Pemasukan") : _("Pengeluaran")})` }))}
          ...
          <div>
            <Label><Trans>Tipe</Trans></Label>
            <Select
              options={[
                { value: 'income', label: _("Pemasukan") },
                { value: 'expense', label: _("Pengeluaran") },
              ]}
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <Label><Trans>Jumlah</Trans></Label>
            ...
            <div>
              <Label><Trans>Tanggal Transaksi</Trans></Label>
          ...
          <div>
            ...
            <TextArea placeholder={_("Catatan (opsional)")} value={formData.note || ''} onChange={(val) => setFormData({ ...formData, note: val })} rows={3} />
          </div>
        </div>
      ...
      <Modal isOpen={showDeleteModal} onClose={() => setShowDeleteModal(false)} className="max-w-sm p-6">
        <h4 className="mb-2 text-lg font-semibold text-gray-800 dark:text-white/90"><Trans>Hapus Transaksi</Trans></h4>
        <p className="mb-6 text-sm text-gray-500 dark:text-gray-400">
          <Trans>Apakah Anda yakin ingin menghapus transaksi keuangan ini?</Trans>
        </p>
        <div className="flex justify-end gap-3">
