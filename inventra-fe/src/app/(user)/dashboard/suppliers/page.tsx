...
export default function Suppliers() {
  ...
  const fetchSuppliers = useCallback(async () => {
    ...
    try {
      ...
      if (res.status) {
        ...
        setSuppliers([]);
        toast.error(res.message || _(msg`Gagal memuat data supplier`));
      }
    } catch {
      toast.error(_(msg`Gagal memuat data supplier`));
    } finally {
      setLoading(false);
    }
  }, [currentPage]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res.status) {
        toast.success(editingSupplier ? _(msg`Supplier berhasil diperbarui`) : _(msg`Supplier berhasil ditambahkan`));
        setShowFormModal(false);
        resetForm();
        fetchSuppliers();
      } else {
        toast.error(res.message || _(msg`Gagal menyimpan supplier`));
      }
    } catch {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (res.status) {
        toast.success(_(msg`Supplier berhasil dihapus`));
        setShowDeleteModal(false);
        setDeletingSupplier(null);
        fetchSuppliers();
      } else {
        toast.error(res.message || _(msg`Gagal menghapus supplier`));
      }
    } catch {
