...
export default function Products() {
  ...
  const fetchProducts = useCallback(async () => {
    ...
    try {
      ...
      if (res.status) {
        ...
        setProducts([]);
        toast.error(res.message || _("Gagal memuat data produk"));
      }
    } catch {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res.status) {
        toast.success(editingProduct ? _("Produk berhasil diperbarui") : _("Produk berhasil ditambahkan"));
        setShowFormModal(false);
        resetForm();
        fetchProducts();
      } else {
        toast.error(res.message || _("Gagal menyimpan produk"));
      }
    } catch {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (res.status) {
        toast.success(_("Produk berhasil dihapus"));
        setShowDeleteModal(false);
        setDeletingProduct(null);
        fetchProducts();
      } else {
        toast.error(res.message || _("Gagal menghapus produk"));
      }
    } catch {
    ...
    <PermissionWrapper permission="Lihat Produk" breadcrumb="Produk">
      <div className='flex flex-col gap-4 '>
        ...
        <div className="mb-4 flex justify-end gap-3">
          ...
          <Can permission="Tambah Produk">
            <Button size="sm" onClick={openCreateModal}>+ <Trans>Tambah Produk</Trans></Button>
          </Can>
        </div>
      </div>
          ...
          <div className="min-w-[1102px]">
            <Table>
              <TableHeader className="border-b border-gray-100 dark:border-white/[0.05]">
                <TableRow>
                  <TableCell isHeader className="px-5 py-3 font-medium text-gray-500 text-start text-theme-xs dark:text-gray-400"><Trans>Nama</Trans></TableCell>
                  ...
                  <TableCell isHeader className="px-5 py-3 font-medium text-gray-500 text-start text-theme-xs dark:text-gray-400"><Trans>Harga Jual</Trans></TableCell>
                  ...
                  <TableCell isHeader className="px-5 py-3 font-medium text-gray-500 text-start text-theme-xs dark:text-gray-400"><Trans>Satuan</Trans></TableCell>
              ...
              <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
                {loading ? (
                  ...
                  <TableRow>
                    <TableCell className="px-5 py-8 text-center text-gray-500"><Trans>Tidak ada data produk</Trans></TableCell>
                  </TableRow>
                ) : (
                  prod.map((product) => (
      ...
      <Modal isOpen={showFormModal} onClose={() => { setShowFormModal(false); resetForm(); }} className="max-w-lg p-6 lg:p-10">
        <h4 className="mb-6 text-lg font-semibold text-gray-800 dark:text-white/90">
          {editingProduct ? <Trans>Edit Produk</Trans> : <Trans>Tambah Produk</Trans>}
        </h4>
        <div className="space-y-4">
          <div>
            <Label><Trans>Gambar Produk</Trans></Label>
              ...
              >
                {({ open }) => (
                  ...
                  >
                    ...
                    <span><Trans>Upload Gambar</Trans></span>
                  </button>
                )}
              </CldUploadWidget>
            </div>
          ...
          <div>
            <Label><Trans>Nama Produk</Trans></Label>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <Label><Trans>Kategori</Trans></Label>
              <Select
                options={categories.map((c) => ({ value: String(c.id), label: c.name }))}
                placeholder={_("Pilih kategori")}
                defaultValue={formData.category_id ? String(formData.category_id) : ''}
                onChange={(v) => setFormData({ ...formData, category_id: Number(v) })}
              />
            </div>
            <div>
              <Label><Trans>Tipe Produk</Trans></Label>
              <Select
                options={[{ value: 'barang', label: _("Barang") }, { value: 'kuliner', label: _("Kuliner") }]}
        ...
        <div className="mt-6 flex justify-end gap-3">
          ...
          <Button size="sm" onClick={handleSubmit} disabled={submitting}>{submitting ? <Trans>Menyimpan...</Trans> : <Trans>Simpan</Trans>}</Button>
        </div>
      </Modal>

      {/* Delete Modal */}
      <Modal isOpen={showDeleteModal} onClose={() => setShowDeleteModal(false)} className="max-w-sm p-6">
        <h4 className="mb-2 text-lg font-semibold text-gray-800 dark:text-white/90"><Trans>Hapus Produk</Trans></h4>
        <p className="mb-6 text-sm text-gray-500 dark:text-gray-400">
          <Trans>Apakah Anda yakin ingin menghapus produk</Trans> <strong>{deletingProduct?.name}</strong>?
        </p>
        <div className="flex justify-end gap-3">
          <Button variant="outline" size="sm" onClick={() => setShowDeleteModal(false)}><Trans>Batal</Trans></Button>
          <Button size="sm" onClick={handleDelete} disabled={submitting} className="bg-error-500 hover:bg-error-600">
            {submitting ? <Trans>Menghapus...</Trans> : <Trans>Hapus</Trans>}
          </Button>
        </div>
      </Modal>
      ...
      <Modal isOpen={showDetailModal} onClose={() => setShowDetailModal(false)} className="max-w-sm p-6 lg:p-8">
        <h4 className="mb-4 text-lg font-semibold text-gray-800 dark:text-white/90 text-center"><Trans>Detail Produk</Trans></h4>
        {detailProduct && (
          <div className="flex flex-col items-center gap-5">
            ...
            <div className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm flex flex-col items-center w-full max-w-[200px]">
              <p className="text-[10px] font-semibold mb-3 text-gray-400 uppercase tracking-wider"><Trans>QR Code Produk</Trans></p>
