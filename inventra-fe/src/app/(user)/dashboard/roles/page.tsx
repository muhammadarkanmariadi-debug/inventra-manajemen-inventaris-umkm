...
export default function Roles() {
  ...
  const fetchRoles = useCallback(async () => {
    ...
    try {
      ...
      if (res.status) {
        ...
        setRoles([]);
        toast.error(res.message || _("Gagal memuat data role"));
      }
    } catch {
      toast.error(_("Gagal memuat data role"));
    } finally {
      setLoading(false);
    }
  }, [currentPage]);
  ...
  const openEditModal = async (role: Role) => {
    try {
      ...
      toast.error(_("Gagal memuat detail role"));
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res.status) {
        toast.success(editingRole ? _("Role berhasil diperbarui") : _("Role berhasil ditambahkan"));
        setShowFormModal(false);
        resetForm();
        fetchRoles();
      } else {
        toast.error(res.message || _("Gagal menyimpan role"));
      }
    } catch {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (res.status) {
        toast.success(_("Role berhasil dihapus"));
        setShowDeleteModal(false);
        setDeletingRole(null);
        fetchRoles();
      } else {
        toast.error(res.message || _("Gagal menghapus role"));
      }
    } catch {
  ...
  const filterConfig = {
    ...
    searchPlaceholder: _("Cari role berdasarkan nama..."),
  };
    ...
    <PermissionWrapper permission="Lihat Peran" breadcrumb="Roles">
      ...
      <div className='flex flex-col gap-4 mb-4'>
        ...
        <div className="flex justify-end gap-3">
          ...
          <Can permission="Tambah Peran">
            <Button size="sm" onClick={openCreateModal}>+ <Trans>Tambah Role</Trans></Button>
          </Can>
        </div>
      </div>
        ...
        <div className="max-w-full overflow-x-auto">
          <Table>
            <TableHeader className="border-b border-gray-100 dark:border-white/[0.05]">
              <TableRow>
                <TableCell isHeader className="px-5 py-3 font-medium text-gray-500 text-start text-theme-xs dark:text-gray-400"><Trans>Nama Role</Trans></TableCell>
            ...
            <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
              {loading ? (
                ...
                <TableRow>
                  <TableCell className="px-5 py-8 text-center text-gray-500" colSpan={3}>
                    <Trans>Tidak ada data role</Trans>
                  </TableCell>
                </TableRow>
              ) : (
      ...
      >
        <h4 className="mb-6 text-lg font-semibold text-gray-800 dark:text-white/90">
          {editingRole ? <Trans>Edit Role</Trans> : <Trans>Tambah Role</Trans>}
        </h4>

        <div className="space-y-4">
          <div>
            <Label><Trans>Nama Role</Trans></Label>
            <Input
              type="text"
              placeholder={_("Nama role")}
          ...
          {editingRole && Object.keys(groupedPermissions).length > 0 && (
            <div>
                  ...
                  return (
                    <div key={module} className="rounded-lg border border-gray-100 dark:border-white/[0.08] p-3">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        >
                          {allChecked ? <Trans>Hapus Semua</Trans> : <Trans>Pilih Semua</Trans>}
                        </button>
                      </div>
              ...
              <p className="mt-2 text-xs text-gray-400 dark:text-gray-500">
                {formData.permissions.length} {_("dari")} {allPermissions.length || Object.values(groupedPermissions).flat().length} {_("permission dipilih")}
              </p>
            </div>
          )}
        </div>
      ...
      >
        ...
        {detailRole?.permissions && detailRole.permissions.length > 0 ? (
          ...
          <p className="text-sm text-gray-500"><Trans>Tidak ada permissions</Trans></p>
        )}
      ...
      <Modal isOpen={showDeleteModal} onClose={() => setShowDeleteModal(false)} className="max-w-sm p-6">
        <h4 className="mb-2 text-lg font-semibold text-gray-800 dark:text-white/90"><Trans>Hapus Role</Trans></h4>
        <p className="mb-6 text-sm text-gray-500 dark:text-gray-400">
          <Trans>Apakah Anda yakin ingin menghapus role</Trans> <strong>{deletingRole?.name}</strong>?
        </p>
        <div className="flex justify-end gap-3">
