...
export default function StockPrediction() {
  ...
  const fetchProducts = useCallback(async () => {
    ...
    try {
      ...
      toast.error(_("Gagal memuat data produk"));
    } finally {
      setLoading(false);
    }
  }, [_]);
  ...
  const handlePredict = async () => {
    if (!selectedProductId) {
      toast.error(_("Harap pilih produk terlebih dahulu"));
      return;
    }
    ...
    try {
      ...
      if (!json.status || !json.data || json.data.length < 2) {
        setPredError(
          json.message ||
          _("Data historis tidak cukup. Minimal diperlukan 2 hari transaksi dalam 30 hari terakhir.")
        );
      ...
      const errMsg =
        ...
        _("Gagal menghubungi API prediksi. Pastikan server FastAPI sedang berjalan.");
      toast.error(errMsg);
    } finally {
      setPredicting(false);
      setFetchingHistory(false);
  ...
  const getStockStatus = (stock: number) => {
    if (stock === 0) return { label: _("Habis"), color: 'error' as const, priority: 0 };
    if (stock <= 5) return { label: _("Kritis"), color: 'error' as const, priority: 1 };
    if (stock <= 15) return { label: _("Rendah"), color: 'warning' as const, priority: 2 };
    if (stock <= 50) return { label: _("Sedang"), color: 'info' as const, priority: 3 };
    return { label: _("Aman"), color: 'success' as const, priority: 4 };
  };
  ...
  const filterConfig = {
    tabs: [
      { label: _("Semua"), value: 'all' },
      { label: _("Kritis"), value: 'critical' },
    ...
    <PermissionWrapper permission="Lihat Transaksi Stok" breadcrumb="Prediksi & Analisis Stok">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3 mb-6">
        <div className="rounded-2xl border border-error-200 bg-error-50 p-5 dark:border-error-800 dark:bg-error-500/10">
          ...
          <p className="mt-1 text-sm text-error-600 dark:text-error-400">
            <Trans>Produk Stok Kritis (&lt;=5)</Trans>
          </p>
        </div>
        <div className="rounded-2xl border border-warning-200 bg-warning-50 p-5 dark:border-warning-800 dark:bg-warning-500/10">
          ...
          <p className="mt-1 text-sm text-warning-600 dark:text-warning-400">
            <Trans>Produk Stok Rendah (6-15)</Trans>
          </p>
        </div>
        <div className="rounded-2xl border border-success-200 bg-success-50 p-5 dark:border-success-800 dark:bg-success-500/10">
          ...
          <p className="mt-1 text-sm text-success-600 dark:text-success-400">
            <Trans>Produk Stok Aman (&gt; 15)</Trans>
          </p>
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03] mb-6">
        <div className="p-5 border-b border-gray-200 dark:border-gray-800">
          <h4 className="text-lg font-semibold text-gray-800 dark:text-white/90">
            <Trans>Status Stok Seluruh Produk</Trans>
          </h4>
        </div>
            ...
            <Table>
              <TableHeader className="border-b border-gray-100 dark:border-white/[0.05]">
                <TableRow>
                  ...
                  <TableCell isHeader className="px-5 py-3 font-medium text-gray-500 text-start text-theme-xs dark:text-gray-400"><Trans>Kategori</Trans></TableCell>
                  <TableCell isHeader className="px-5 py-3 font-medium text-gray-500 text-start text-theme-xs dark:text-gray-400"><Trans>Stok</Trans></TableCell>
                  ...
                  <TableCell isHeader className="px-5 py-3 font-medium text-gray-500 text-start text-theme-xs dark:text-gray-400"><Trans>Tipe</Trans></TableCell>
                </TableRow>
              </TableHeader>
                ...
                {loading ? (
                  ...
                  <TableRow>
                    <TableCell className="px-5 py-8 text-center text-gray-500">
                      <Trans>Tidak ada data produk</Trans>
                    </TableCell>
                  </TableRow>
                ) : (
      ...
      >
        <div className="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/[0.03] mb-6">
          <div className="mb-5">
            <h4 className="text-lg font-semibold text-gray-800 dark:text-white/90">
              <Trans>Prediksi Stok (FastAPI)</Trans>
            </h4>
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
            <Trans>Prediksi tren penjualan menggunakan Linear Regression + Prophet berdasarkan data transaksi 30 hari terakhir.</Trans>
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 mb-3">
          <div>
            ...
            >
              <option value="">{_("-- Pilih Produk --")}</option>
              {products.map((p) => (
        ...
        <p className="text-xs text-gray-400 dark:text-gray-500 mb-5">
          <Trans>Data historis diambil otomatis dari transaksi keluar (OUT) 30 hari terakhir.</Trans>
        </p>
          ...
          <Button size="sm" onClick={handlePredict} disabled={predicting}>
            {predicting ? (
              <span className="flex items-center gap-2">
                ...
                {fetchingHistory
                  ? <Trans>Mengambil data historis...</Trans>
                  : <Trans>Memprediksi...</Trans>
                }
              </span>
            ) : (
              <Trans>Jalankan Prediksi</Trans>
            )}
          </Button>
        ...
        {predicting && (
          <div className="flex flex-col items-center justify-center py-12">
            ...
            <p className="text-sm text-gray-500">
              {fetchingHistory
                ? <Trans>Mengambil data historis transaksi...</Trans>
                : <Trans>Menjalankan model prediksi...</Trans>
              }
            </p>
          </div>
        )}
        ...
        {!predicting && predResults.length > 0 && predResults.map((result) => {
            ...
            <div key={result.product_id} className="mb-8">
              ...
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 mb-5">
                <div className="rounded-xl border border-gray-100 bg-gray-50 p-4 dark:border-gray-700 dark:bg-gray-800/50">
                  <p className="text-xs text-gray-500 dark:text-gray-400 mb-1"><Trans>Stok Saat Ini</Trans></p>
                  <p className="text-2xl font-bold text-gray-800 dark:text-white/90">{result.current_stock}</p>
                </div>
                <div className="rounded-xl border border-gray-100 bg-gray-50 p-4 dark:border-gray-700 dark:bg-gray-800/50">
                  <p className="text-xs text-gray-500 dark:text-gray-400 mb-1"><Trans>Rata-rata Penjualan/Hari</Trans></p>
                ...
                <div className="rounded-xl border border-gray-100 bg-gray-50 p-4 dark:border-gray-700 dark:bg-gray-800/50">
                  <p className="text-xs text-gray-500 dark:text-gray-400 mb-1"><Trans>Tren Penjualan</Trans></p>
                  <div className="mt-1">
              ...
              <div className="rounded-xl border border-brand-200 bg-brand-50 p-4 mb-5 dark:border-brand-800 dark:bg-brand-500/10">
                <p className="text-sm font-medium text-brand-700 dark:text-brand-300"><Trans>Rekomendasi</Trans></p>
              ...
              <div className="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] mb-5">
                <h6 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3">
                  <Trans>Grafik Prediksi Penjualan</Trans>
                </h6>
                <ReactApexChart
                  type="line"
                  height={280}
              ...
              <div className="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
                <div className="p-4 border-b border-gray-100 dark:border-white/[0.05]">
                  <h6 className="text-sm font-semibold text-gray-700 dark:text-gray-300">
                    <Trans>Tabel Forecast Harian</Trans>
                  </h6>
                </div>
                <div className="max-w-full overflow-x-auto">
                    ...
                    <Table>
                      <TableHeader className="border-b border-gray-100 dark:border-white/[0.05]">
                        <TableRow>
                          ...
                          <TableCell isHeader className="px-5 py-3 font-medium text-gray-500 text-start text-theme-xs dark:text-gray-400"><Trans>Prediksi (yhat)</Trans></TableCell>
                          <TableCell isHeader className="px-5 py-3 font-medium text-gray-500 text-start text-theme-xs dark:text-gray-400"><Trans>Batas Bawah</Trans></TableCell>
                          <TableCell isHeader className="px-5 py-3 font-medium text-gray-500 text-start text-theme-xs dark:text-gray-400"><Trans>Batas Atas</Trans></TableCell>
                        </TableRow>
                      </TableHeader>
                      <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
          ...
          {!predicting && predResults.length === 0 && !predError && (
            <div className="flex items-center justify-center py-8 text-gray-400 text-sm">
              <Trans>Pilih produk lalu klik &quot;Jalankan Prediksi&quot; untuk melihat hasil prediksi.</Trans>
            </div>
          )}
        </div>
      </FeatureGate>
