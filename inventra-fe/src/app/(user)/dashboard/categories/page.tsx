...
export default function Categories() {
  ...
  const fetchCategories = useCallback(async () => {
    ...
    try {
      ...
      if (res.status) {
        ...
        setCategories([]);
        toast.error(res.message || _("Gagal memuat data kategori"));
      }
    } catch {
      toast.error(_("Gagal memuat data kategori"));
    } finally {
      setLoading(false);
    }
  }, [currentPage]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res.status) {
        toast.success(editingCategory ? _("Kategori berhasil diperbarui") : _("Kategori berhasil ditambahkan"));
        setShowFormModal(false);
        resetForm();
        fetchCategories();
      } else {
        toast.error(res.message || _("Gagal menyimpan kategori"));
      }
    } catch {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (res.status) {
        toast.success(_("Kategori berhasil dihapus"));
        setShowDeleteModal(false);
        setDeletingCategory(null);
        fetchCategories();
      } else {
        toast.error(res.message || _("Gagal menghapus kategori"));
      }
    } catch {
  ...
  const filterConfig = {
    ...
    searchPlaceholder: _("Cari kategori berdasarkan nama..."),
  };
      ...
      <div>
        ...
        <div className='flex flex-col gap-4 mb-4'>
          ...
          <div className="flex justify-end gap-3">
            ...
            <Can permission="Tambah Kategori">
              <Button size="sm" onClick={openCreateModal}>+ <Trans>Tambah Kategori</Trans></Button>
            </Can>
          </div>
        </div>
            ...
            <Table>
              <TableHeader className="border-b border-gray-100 dark:border-white/[0.05]">
                <TableRow>
                  ...
                  <TableCell isHeader className="px-5 py-3 font-medium text-gray-500 text-start text-theme-xs dark:text-gray-400"><Trans>Deskripsi</Trans></TableCell>
                  <TableCell isHeader className="px-5 py-3 font-medium text-gray-500 text-start text-theme-xs dark:text-gray-400"><Trans>Jumlah Produk</Trans></TableCell>
              ...
              <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
                {loading ? (
                  ...
                  <TableRow><TableCell className="px-5 py-8 text-center text-gray-500"><Trans>Tidak ada data kategori</Trans></TableCell></TableRow>
                ) : (
        ...
        <Modal isOpen={showFormModal} onClose={() => { setShowFormModal(false); resetForm(); }} className="max-w-md p-6 lg:p-10">
          <h4 className="mb-6 text-lg font-semibold text-gray-800 dark:text-white/90">
            {editingCategory ? <Trans>Edit Kategori</Trans> : <Trans>Tambah Kategori</Trans>}
          </h4>
          <div className="space-y-4">
            <div>
              <Label><Trans>Nama Kategori</Trans></Label>
              <Input type="text" placeholder={_("Nama kategori")} defaultValue={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} />
            </div>
            <div>
              <Label><Trans>Deskripsi</Trans></Label>
              <TextArea placeholder={_("Deskripsi kategori (opsional)")} value={formData.description || ''} onChange={(val) => setFormData({ ...formData, description: val })} rows={3} />
            </div>
          </div>
        ...
        <Modal isOpen={showDeleteModal} onClose={() => setShowDeleteModal(false)} className="max-w-sm p-6">
          <h4 className="mb-2 text-lg font-semibold text-gray-800 dark:text-white/90"><Trans>Hapus Kategori</Trans></h4>
          <p className="mb-6 text-sm text-gray-500 dark:text-gray-400">
            <Trans>Apakah Anda yakin ingin menghapus kategori</Trans> <strong>{deletingCategory?.name}</strong>?
          </p>
          <div className="flex justify-end gap-3">
