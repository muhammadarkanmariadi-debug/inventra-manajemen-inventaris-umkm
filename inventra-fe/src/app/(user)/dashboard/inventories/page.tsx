...
export default function InventoriesPage() {
  ...
  const filterConfig: Required<Pick<FilterBarProps, "tabs" | "selects" | "sorts" | "searchPlaceholder">> = {
    tabs: [
      ...
      { label: _("Ready"), value: "READY" },
      { label: _("On Hold"), value: "ON_HOLD" },
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      toast.error(err?.message || _("Gagal memuat data inventaris."));
    } finally {
      setLoading(false);
    }
  ...
  const generateSuratJalan = async () => {
    ...
    try {
      ...
      toast.success(_("Surat Jalan berhasil diunduh"));
      setShowSJModal(false);
    } catch (err) {
      console.error(err);
      toast.error(_("Gagal generate Surat Jalan"));
    } finally {
  ...
  return (
    <PermissionWrapper permission="Lihat Produk" breadcrumb="Stok Inventaris">
            ...
            <div className="min-w-[1102px]">
              <Table>
                <TableHeader className="border-b border-gray-100 dark:border-white/[0.05]">
                  <TableRow>
                    <TableCell isHeader className="px-5 py-3 font-medium text-gray-500 text-start text-theme-xs dark:text-gray-400"><Trans>Kode Inventaris</Trans></TableCell>
                    <TableCell isHeader className="px-5 py-3 font-medium text-gray-500 text-start text-theme-xs dark:text-gray-400"><Trans>Produk</Trans></TableCell>
                    <TableCell isHeader className="px-5 py-3 font-medium text-gray-500 text-start text-theme-xs dark:text-gray-400"><Trans>Jumlah</Trans></TableCell>
                    <TableCell isHeader className="px-5 py-3 font-medium text-gray-500 text-start text-theme-xs dark:text-gray-400"><Trans>Status</Trans></TableCell>
                    <TableCell isHeader className="px-5 py-3 font-medium text-gray-500 text-start text-theme-xs dark:text-gray-400"><Trans>Lokasi</Trans></TableCell>
                ...
                <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
                  {loading ? (
                    ...
                    <TableRow>
                      <TableCell className="px-5 py-8 text-center text-gray-500" colSpan={7}><Trans>Tidak ada data inventaris.</Trans></TableCell>
                    </TableRow>
                  ) : (
                    filtered.map(inv => (
                      <TableRow key={inv.id}>
                        ...
                        <TableCell className="px-4 py-3 text-start">
                          <div className="flex items-center gap-2">
                            ...
                            <button onClick={() => router.push(`/dashboard/scan?code=${inv.inventory_code}`)} className="text-brand-500 hover:text-brand-700 text-sm"><Trans>Scan</Trans></button>
                            <button onClick={() => { setQrCodeData({ code: inv.inventory_code, title: _("QR Inventaris"), subtitle: `${_("Produk")}: ${inv.product?.name || '-'}` }); setQrModalOpen(true); }} className="text-gray-500 hover:text-gray-700 text-sm border border-gray-200 px-2 py-0.5 rounded ml-1"><Trans>Cetak QR</Trans></button>
                            <button
      ...
      <Modal isOpen={showDetailModal} onClose={() => { setShowDetailModal(false); setDetailItem(null); }} className="max-w-xl">
        <div className="p-5">
          <h4 className="mb-4 text-lg font-semibold text-gray-800 dark:text-white"><Trans>Detail Inventaris</Trans></h4>
          {detailLoading ? (
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <p className="text-xs text-gray-500 dark:text-gray-400"><Trans>Kode</Trans></p>
                  <p className="font-mono font-semibold text-gray-800 dark:text-white">{detailItem.inventory_code}</p>
                </div>
                <div>
                  <p className="text-xs text-gray-500 dark:text-gray-400"><Trans>Produk</Trans></p>
                  <p className="font-semibold text-gray-800 dark:text-white">{detailItem.product?.name || '-'}</p>
                </div>
                <div>
                  <p className="text-xs text-gray-500 dark:text-gray-400"><Trans>Jumlah</Trans></p>
                  <p className="font-semibold text-gray-800 dark:text-white">{detailItem.quantity}</p>
                </div>
                <div>
                  <p className="text-xs text-gray-500 dark:text-gray-400"><Trans>Status</Trans></p>
                ...
                <div>
                  <p className="text-xs text-gray-500 dark:text-gray-400"><Trans>Lokasi</Trans></p>
                  <p className="text-gray-700 dark:text-gray-300">{detailItem.location?.name || '-'}</p>
                </div>
                <div>
                  <p className="text-xs text-gray-500 dark:text-gray-400"><Trans>Dibuat</Trans></p>
            ...
            <p className="text-red-500"><Trans>Gagal memuat detail.</Trans></p>
          )}
        </div>
      </Modal>
      ...
      <Modal isOpen={showSJModal} onClose={() => setShowSJModal(false)} className="max-w-md p-6 lg:p-10">
        ...
        <div className="space-y-4">
          <div>
            <Label><Trans>Nama Penerima</Trans></Label>
            <Input type="text" placeholder={_("Nama penerima")} defaultValue={sjForm.receiverName} onChange={(e) => setSjForm({ ...sjForm, receiverName: e.target.value })} />
          </div>
          <div>
            <Label><Trans>Alamat Penerima</Trans></Label>
            <Input type="text" placeholder={_("Alamat penerima")} defaultValue={sjForm.receiverAddress} onChange={(e) => setSjForm({ ...sjForm, receiverAddress: e.target.value })} />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <Label><Trans>No. Telepon</Trans></Label>
              <Input type="text" placeholder={_("No. telepon")} defaultValue={sjForm.receiverPhone} onChange={(e) => setSjForm({ ...sjForm, receiverPhone: e.target.value })} />
            </div>
            <div>
              <Label><Trans>Kendaraan</Trans></Label>
              <Input type="text" placeholder={_("Kendaraan pengiriman")} defaultValue={sjForm.vehicle} onChange={(e) => setSjForm({ ...sjForm, vehicle: e.target.value })} />
            </div>
          </div>
          <div>
            <Label><Trans>Catatan</Trans></Label>
            <Input type="text" placeholder={_("Catatan tambahan")} defaultValue={sjForm.notes} onChange={(e) => setSjForm({ ...sjForm, notes: e.target.value })} />
          </div>
        </div>
        <div className="mt-6 flex justify-end gap-3">
          <Button variant="outline" size="sm" onClick={() => setShowSJModal(false)}><Trans>Batal</Trans></Button>
          <Button size="sm" onClick={generateSuratJalan} disabled={sjGenerating}>
            {sjGenerating ? <Trans>Generating...</Trans> : <Trans>Cetak & Unduh PDF</Trans>}
          </Button>
        </div>
      </Modal>
    </PermissionWrapper>
