...
export default function StockAdjustmentPage() {
  ...
  const filterConfig: Required<Pick<FilterBarProps, "tabs" | "selects" | "searchPlaceholder">> = {
    tabs: [
      { label: _("Semua"), value: "Semua" },
      { label: _("Masuk"), value: "IN" },
      { label: _("Keluar"), value: "OUT" },
    ],
    selects: [],
    searchPlaceholder: _("Cari transaksi..."),
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (res.status) {
      toast.success(_("Penyesuaian stok berhasil diproses!"));
      ...
      <div className='flex flex-col gap-4 '>
        ...
        <div className="mb-4 flex justify-end gap-3">
          ...
          <Can permission="Tambah Transaksi Stok">
            <Button size="sm" onClick={() => setOpenCreateModal(true)}>+ <Trans>Tambah Penyesuaian</Trans></Button>
          </Can>
        </div>
      </div>
            ...
            <Table>
              <TableHeader className="border-b border-gray-100 dark:border-white/[0.05]">
                <TableRow>
                  ...
                  <TableCell isHeader className="px-5 py-3 font-medium text-gray-500 text-start text-theme-xs dark:text-gray-400"><Trans>Kuantitas</Trans></TableCell>
              ...
              <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
                {fetchingHistory ? (
                  ...
                  <TableRow><TableCell className="px-5 py-8 text-center text-gray-500"><Trans>Belum ada riwayat penyesuaian</Trans></TableCell></TableRow>
                ) : (
      ...
      <Modal isOpen={openCreateModal} onClose={() => setOpenCreateModal(false)} className="max-w-3xl">
        <div className="p-6">
          <h4 className="mb-6 text-lg font-semibold text-gray-800 dark:text-white/90">
            <Trans>Tambah Penyesuaian Stok</Trans>
          </h4>
          <form onSubmit={handleSubmit}>
            <div className="mb-6">
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{_("Tipe Penyesuaian")}</label>
              <div className="flex gap-4">
                <label className="flex items-center gap-2 cursor-pointer">
                  ...
                  <span className="text-sm">{_("Penambahan (+)")}</span>
                </label>
                <label className="flex items-center gap-2 cursor-pointer">
                  ...
                  <span className="text-sm">{_("Pengurangan (-)")}</span>
                </label>
              </div>
            </div>
              ...
              {items.map((item, index) => (
                <div key={index} className="flex flex-col gap-4 p-4 border border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50 rounded-lg relative group">
                  ...
                  {type === 'ADJUSTMENT_ADD' ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="flex-1">
                        <label className="block text-xs font-medium text-gray-500 mb-1">{_("Produk")}</label>
                        <Select
                      ...
                      <div className="flex-1">
                        <label className="block text-xs font-medium text-gray-500 mb-1">{_("Lokasi Simpan")}</label>
                        <Select
                          options={locationOptions}
                          value={item.location_id}
                          onChange={(val) => updateItem(index, 'location_id', val)}
                          placeholder={_("Pilih Lokasi")}
                          isSearchable
                        />
                      </div>
                    </div>
                    ...
                    <div className="flex-1">
                      <label className="block text-xs font-medium text-gray-500 mb-1">{_("Batch Inventaris (Ready)")}</label>
                      <Select
                        options={inventoryOptions}
                        value={item.inventory_id}
                        onChange={(val) => updateItem(index, 'inventory_id', val)}
                        placeholder={_("Pilih Batch")}
                        isSearchable
                      />
                    </div>
                  )}
                  ...
                  <div className="w-full md:w-32">
                    <label className="block text-xs font-medium text-gray-500 mb-1">{_("Kuantitas")}</label>
            ...
            <Button type="button" variant="outline" size="sm" onClick={addItem} className="mb-6 mb-8 border-dashed">+ {_("Tambah Item Lain")}</Button>

            <div className="mb-8">
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{_("Catatan")}</label>
              <textarea value={notes} onChange={(e) => setNotes(e.target.value)} className="w-full text-sm p-3 border border-gray-200 rounded-lg dark:bg-gray-800 dark:border-gray-700 h-24 resize-none" placeholder={_("Alasan penyesuaian...")}></textarea>
            </div>
            ...
            <div className="flex justify-end pt-4 border-t border-gray-100 dark:border-gray-800 gap-3">
              ...
              <Button type="submit" disabled={loading || fetchingData} className="px-8">
                {loading ? _("Memproses...") : _("Kirim Penyesuaian")}
              </Button>
            </div>
