  ...
  return (
    <PermissionWrapper permission="Lihat Produk" breadcrumb="Kelola Gudang">
      <div className="flex flex-col gap-4">
        ...
        <div className="mb-4 flex justify-end gap-3">
          ...
          <Can permission="Tambah Produk">
            <Button size="sm" onClick={openCreateModal}>+ <Trans>Tambah Gudang</Trans></Button>
          </Can>
        </div>
            ...
            <div className="min-w-[600px]">
              <Table>
                <TableHeader className="border-b border-gray-100 dark:border-white/[0.05]">
                  <TableRow>
                    <TableCell isHeader className="px-5 py-3 font-medium text-gray-500 text-start text-theme-xs dark:text-gray-400"><Trans>No</Trans></TableCell>
                    <TableCell isHeader className="px-5 py-3 font-medium text-gray-500 text-start text-theme-xs dark:text-gray-400"><Trans>Nama Gudang</Trans></TableCell>
                    <TableCell isHeader className="px-5 py-3 font-medium text-gray-500 text-start text-theme-xs dark:text-gray-400"><Trans>Jumlah Inventaris</Trans></TableCell>
                ...
                <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
                  {loading ? (
                    ...
                    <TableRow>
                      <TableCell className="px-5 py-8 text-center text-gray-500" colSpan={4}>
                        <Trans>Tidak ada data Gudang.</Trans>
                      </TableCell>
                    </TableRow>
                  ) : (
      ...
      <Modal isOpen={showFormModal} onClose={() => setShowFormModal(false)} className="max-w-md">
        <div className="p-5">
          <h4 className="mb-4 text-lg font-semibold text-gray-800 dark:text-white">
            {editingLocation ? <Trans>Edit Gudang</Trans> : <Trans>Tambah Gudang Baru</Trans>}
          </h4>
          <div className="mb-4">
            <Label><Trans>Nama Gudang</Trans></Label>
            <Input
              type="text"
              value={formName}
          ...
          <div className="flex justify-end gap-3">
            <Button size="sm" variant="outline" onClick={() => setShowFormModal(false)}><Trans>Batal</Trans></Button>
            <Button size="sm" onClick={handleSubmit} disabled={submitting || !formName.trim()}>
              {submitting ? <Trans>Menyimpan...</Trans> : editingLocation ? <Trans>Perbarui</Trans> : <Trans>Simpan</Trans>}
            </Button>
          </div>
        </div>
      </Modal>
      ...
      <Modal isOpen={showDeleteModal} onClose={() => { setShowDeleteModal(false); setDeletingLocation(null); }} className="max-w-md">
        <div className="p-5">
          <h4 className="mb-2 text-lg font-semibold text-gray-800 dark:text-white"><Trans>Hapus Gudang</Trans></h4>
          <p className="mb-4 text-sm text-gray-600 dark:text-gray-400">
            <Trans>Apakah Anda yakin ingin menghapus Gudang</Trans> <strong>{deletingLocation?.name}</strong>?
            {deletingLocation && deletingLocation.inventories_count > 0 && (
              <span className="mt-1 block text-red-500">⚠ <Trans>Gudang ini memiliki</Trans> {deletingLocation.inventories_count} <Trans>inventaris. Hapus akan ditolak.</Trans></span>
            )}
          </p>
          <div className="flex justify-end gap-3">
            <Button size="sm" variant="outline" onClick={() => { setShowDeleteModal(false); setDeletingLocation(null); }}><Trans>Batal</Trans></Button>
            <Button size="sm" onClick={handleDelete} disabled={submitting}>
              {submitting ? <Trans>Menghapus...</Trans> : <Trans>Ya, Hapus</Trans>}
            </Button>
          </div>
        </div>
      </Modal>
