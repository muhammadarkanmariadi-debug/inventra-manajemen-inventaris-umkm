      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 mt-4">
        <div className="lg:col-span-5 space-y-6">
          <div className="bg-white dark:bg-gray-800/50 p-6 rounded-xl border border-gray-200 dark:border-gray-800 shadow-sm">
            <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4"><Trans>Scan QR Code</Trans></h3>
            <QRScanner onScan={(code) => scan(code)} />

            <div className="mt-6 pt-4 border-t  border-gray-100 dark:border-gray-800">
              <p className="text-sm font-medium text-gray-600 dark:text-gray-400 mb-2"><Trans>Atau masukkan kode manual:</Trans></p>
              <form onSubmit={handleManualScan} className="gap-2 md:flex-row flex flex-col">
                <input
                  type="text"
                  value={manualCode}
                  onChange={(e) => setManualCode(e.target.value)}
                  placeholder={_("Mis. INV-A1B2C3")}
            ...
            {loading && <p className="text-sm text-brand-500 mt-4 text-center animate-pulse"><Trans>Memproses...</Trans></p>}
          </div>

          {inventory && (
            <div className="bg-white dark:bg-gray-800/50 p-6 rounded-xl border border-gray-200 dark:border-gray-800 shadow-sm">
              <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4"><Trans>Detail Inventaris</Trans></h3>

              <div className="flex flex-col gap-3">
                <div className="flex justify-between items-center py-2 border-b border-gray-100 dark:border-gray-700">
                  <span className="text-sm text-gray-500"><Trans>Produk</Trans></span>
                  <span className="font-medium text-gray-900 dark:text-white">{inventory.product?.name}</span>
                </div>
                <div className="flex justify-between items-center py-2 border-b border-gray-100 dark:border-gray-700">
                  <span className="text-sm text-gray-500"><Trans>Kode QR / Batch</Trans></span>
                  <span className="font-medium text-brand-600 bg-brand-50 px-2 py-1 rounded-md">{inventory.inventory_code}</span>
                </div>
                <div className="flex justify-between items-center py-2 border-b border-gray-100 dark:border-gray-700">
                  <span className="text-sm text-gray-500"><Trans>Kuantitas</Trans></span>
                ...
                <div className="flex justify-between items-center py-2 border-b border-gray-100 dark:border-gray-700">
                  <span className="text-sm text-gray-500"><Trans>Lokasi</Trans></span>
                  <span className="font-bold text-gray-900 dark:text-white text-lg">{inventory.location?.name || '-'}</span>
                </div>
                <div className="flex justify-between items-center py-2">
                  <span className="text-sm text-gray-500"><Trans>Status Saat Ini</Trans></span>
        ...
        <div className="lg:col-span-7">
          {inventory ? (
            <div className="bg-white dark:bg-gray-800/50 p-6 rounded-xl border border-gray-200 dark:border-gray-800 shadow-sm sticky top-24">
              <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-6"><Trans>Riwayat Rekam Jejak (Logs)</Trans></h3>
              
              {/* AI Prediction Widget */}
              <FeatureGate
                featureKey="qc_workflow"
                planRequired="Professional"
                title="AI QC Defect Predictor Terkunci"
                description="Fitur prediksi cacat dan rekomendasi inspeksi kualitas otomatis membutuhkan paket Professional atau Enterprise."
              >
                {predicting && (
                  <div className="mb-6 p-4 rounded-xl border border-brand-200 bg-brand-50/50 dark:border-brand-800/50 dark:bg-brand-500/5 flex items-center gap-3">
                     <p className="text-brand-600 text-sm animate-pulse"><Trans>Menghitung Prediksi QC (AI)...</Trans></p>
                  </div>
                )}
                  ...
                  <div className={`mb-6 p-4 rounded-xl border ${
                    ...
                    <div className="flex justify-between items-start mb-2">
                      <h4 className="text-sm font-bold flex items-center gap-2">
                        <span className="text-xl">✨</span> {/* @ts-ignore */}<Trans>AI QC Defect Predictor</Trans></h4>
                      <Badge size="sm" color={
                         defectPrediction.risk_level === 'HIGH' ? 'error' :
                         defectPrediction.risk_level === 'MEDIUM' ? 'warning' : 'success'
                      }>
                        {defectPrediction.risk_level} {/* @ts-ignore */}<Trans>RISK</Trans></Badge>
                    </div>
            ...
            <div className="bg-gray-50 dark:bg-gray-800/30 p-10 rounded-xl border border-dashed border-gray-200 dark:border-gray-800 h-full flex flex-col items-center justify-center text-center">
              ...
              <h4 className="text-gray-500 dark:text-gray-400 font-medium mb-1"><Trans>Tidak ada data</Trans></h4>
              <p className="text-sm text-gray-400 dark:text-gray-500"><Trans>Pindai kode QR untuk melihat rekam jejak inventaris.</Trans></p>
            </div>
          )}
        </div>
      </div>
