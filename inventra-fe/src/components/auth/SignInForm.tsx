  ...
  return (
    ...
    lg:w-1/2 w-full">
        ...
        <div className="w-full max-w-md sm:pt-10 mx-auto mb-5">
          ...
          >
            <ChevronLeftIcon />
            {/* @ts-ignore */}<Trans>Back to dashboard</Trans></Link>
        </div>
          ...
          <div>
            <div className="mb-5 sm:mb-8">
              <h1 className="mb-2 font-semibold text-gray-800 text-title-sm dark:text-white/90 sm:text-title-md">
                ...
                {/* @ts-ignore */}<Trans>Sign In</Trans></h1>
              <p className="text-sm text-gray-500 dark:text-gray-400">
                {/* @ts-ignore */}<Trans>Enter your email and password to sign in!</Trans></p>
            </div>
            <div>
              ...
              <form onSubmit={async (e) => {
                ...
                <div className="space-y-6">
                  <div>
                    <Label>
                      {/* @ts-ignore */}<Trans>Email</Trans><span className="text-error-500">*</span>{" "}
                    </Label>
                    <Input defaultValue={data.email} onChange={e => { setData({ ...data, email: e.target.value }) }} placeholder="info@gmail.com" type="email" />
                  </div>
                  <div>
                    <Label>
                      {/* @ts-ignore */}<Trans>Password</Trans><span className="text-error-500">*</span>{" "}
                    </Label>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="block font-normal text-gray-700 text-theme-sm dark:text-gray-400">
                        {/* @ts-ignore */}<Trans>Keep me logged in</Trans></span>
                    </div>
                    <Link
                      href="/reset-password"
                      className="text-sm text-brand-500 hover:text-brand-600 dark:text-brand-400"
                    >
                      {/* @ts-ignore */}<Trans>Forgot password?</Trans></Link>
                  </div>
                  <div>
                    <Button type="submit" className="w-full" size="sm">
                      {/* @ts-ignore */}<Trans>Sign in</Trans></Button>
              ...
              <div className="mt-6 flex items-center justify-between">
                ...
                <span className="text-xs text-center text-gray-500 uppercase dark:text-gray-400">
                  {/* @ts-ignore */}<Trans>BISA JUGA DENGAN</Trans></span>
                <span className="w-1/5 border-b dark:border-gray-600 lg:w-1/4"></span>
              </div>
              <div className="mt-6">
                <button
                ...
                >
                  ...
                  {/* @ts-ignore */}<Trans>Masuk dengan Google</Trans></button>
              </div>

              <div className="mt-5">
                <p className="text-sm font-normal text-center text-gray-700 dark:text-gray-400 sm:text-start">
                  {/* @ts-ignore */}<Trans>Don&apos;t have an account?</Trans>{""}
                  <Link
                    href="/signup"
                    className="text-brand-500 hover:text-brand-600 dark:text-brand-400"
                  >
                    {/* @ts-ignore */}<Trans>Sign Up</Trans></Link>
                </p>
              </div>
            </div>
          </div>
