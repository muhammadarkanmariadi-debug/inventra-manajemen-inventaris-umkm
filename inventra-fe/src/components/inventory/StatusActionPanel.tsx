...
export default function StatusActionPanel({ inventory, onUpdateStatus, options  }: ActionPanelProps) {
  ...
  return (
    <div className="p-4 bg-gray-50 dark:bg-gray-800/50 rounded-xl border border-gray-100 dark:border-gray-800 flex flex-col gap-3">
      <h4 className="text-sm font-semibold text-gray-900 dark:text-white"><Trans>Available Actions</Trans></h4>

      <input
        type="text"
        className="text-sm w-full p-2 border border-gray-200 rounded-lg dark:bg-gray-900 dark:border-gray-700"
        placeholder={_("Add optional notes...")}
        value={notes}
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {status === 'READY' && (
          <>
            ...
            <p className="text-xs text-gray-500 mt-2 block w-full"><Trans>Ready items can be securely deducted via Transactions.</Trans></p>
          </>
        )}

        {status === 'REJECT' && (
          <p className="text-sm text-error-500"><Trans>Item is permanently rejected.</Trans></p>
        )}
      </div>
    </div>
  );
