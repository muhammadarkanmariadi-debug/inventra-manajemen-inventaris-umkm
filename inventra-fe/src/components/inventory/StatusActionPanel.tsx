...
export default function StatusActionPanel({ inventory, onUpdateStatus, options  }: ActionPanelProps) {
  ...
  return (
    <div className="p-4 bg-gray-50 dark:bg-gray-800/50 rounded-xl border border-gray-100 dark:border-gray-800 flex flex-col gap-3">
      <h4 className="text-sm font-semibold text-gray-900 dark:text-white"><Trans id="Available Actions" /></h4>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {status === 'READY' && (
          <>
            ...
            <p className="text-xs text-gray-500 mt-2 block w-full"><Trans id="Ready items can be securely deducted via Transactions." /></p>
          </>
        )}

        {status === 'REJECT' && (
          <p className="text-sm text-error-500"><Trans id="Item is permanently rejected." /></p>
        )}
      </div>
    </div>
  );
