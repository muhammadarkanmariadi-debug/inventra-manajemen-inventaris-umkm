...
export default function InventoryLogTimeline({ logs }: { logs: any[] }) {
  if (!logs || logs.length === 0) {
    return <p className="text-sm text-gray-500"><Trans>No logs found for this batch.</Trans></p>;
  }
      ...
      {logs.map((log: any, index: number) => (
        <div key={log.id} className="mb-6 ml-6">
          ...
          <time className="block mb-2 text-xs font-normal leading-none text-gray-400 dark:text-gray-500">
            {new Date(log.created_at).toLocaleString()} <Trans>oleh</Trans> {log.user?.username || 'System'}
          </time>
          <div className="text-sm font-normal text-gray-500 dark:text-gray-400">
            <p><Trans>Quantity affected:</Trans> <strong>{log.quantity}</strong></p>
            {log.from_status && log.to_status && (
              <p className="mt-1 flex items-center gap-2">
                <Trans>Status:</Trans> <Badge size="sm" color="light">{log.from_status.name}</Badge> → <Badge size="sm" color="info">{log.to_status.name}</Badge>
              </p>
            )}
          ...
          {log.location && (
            <p className="mt-1 flex items-center gap-2">
              <Trans>Lokasi:</Trans> <Badge size="sm" color="light">{log.location.name}</Badge>
            </p>
          )}
        </div>
      ))}
