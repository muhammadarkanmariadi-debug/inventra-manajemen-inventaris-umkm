...
# Try to import Prophet; set a flag if unavailable
try:
    ...
    logger.warning(f"Prophet not available, will use fallback forecasting: {e}")
...
def _simple_forecast(sales_list: List[int], forecast_days: int) -> List[ProphetForecast]:
    """Fallback forecasting menggunakan simple moving average."""
    from datetime import datetime, timedelta
    today = datetime.today()
    avg = np.mean(sales_list) if sales_list else 0
    std = np.std(sales_list) if len(sales_list) > 1 else avg * 0.2
    return [
        ProphetForecast(
            ds=(today + timedelta(days=i+1)).strftime("%Y-%m-%d"),
            yhat=round(max(0, avg), 2),
            yhat_lower=round(max(0, avg - std), 2),
            yhat_upper=round(max(0, avg + std), 2)
        )
...
def run_prophet_forecast(sales_list: List[int], forecast_days: int) -> List[ProphetForecast]:
    if len(sales_list) < 2:
        ...
        return _simple_forecast(sales_list, min(forecast_days, 7))

    # Jika Prophet tidak tersedia, gunakan fallback
    if not PROPHET_AVAILABLE:
        logger.warning("Prophet not available, using simple moving average fallback")
        return _simple_forecast(sales_list, forecast_days)
    ...
    try:
        ...
        logger.error(f"Prophet forecast failed, using fallback: {e}")
        return _simple_forecast(sales_list, forecast_days)
